...
function FlowBuilder() {
    ...
    useEffect(() => {}, [nodes, pathwayID]); // monitor pathwayID for changes
    ...
    const onUpdate = useCallback(() => {
        if (pathwayID) {
            // checks an id exists before trying to update a pathway
            try {
                const pathwayObj = generatePathwayJson(
                    nodes,
                    edges,
    ...
    const onNodeClick = (e, clickedNode) => {
        ...
        if (window.confirm('Do you really want to delete this node?')) {
            ...
            ); // deletes edges with selected node id
        }
    };
